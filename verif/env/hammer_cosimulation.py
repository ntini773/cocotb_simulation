...
try:
    ...
    # Try different memory configuration approaches with fallbacks
    mem_cfg = None
    try:
        # Approach 1: Try mem_cfg_t
        if hasattr(hammer, 'mem_cfg_t') and hasattr(hammer, 'DramBase'):
            mem_cfg = hammer.mem_cfg_t(hammer.DramBase, 256 * 1024 * 1024)
        elif hasattr(hammer, 'MemConfig'):
            # Approach 2: Try MemConfig
            mem_cfg = hammer.MemConfig(256 * 1024 * 1024)
        elif hasattr(hammer, 'memory_config'):
            # Approach 3: Try memory_config function
            mem_cfg = hammer.memory_config(256 * 1024 * 1024)
        else:
            # Approach 4: No memory config - use defaults
            mem_cfg = None
    except Exception:
        # If any memory config fails, just use None
        mem_cfg = None
    ...
    # For UVM context, use the simplest possible Hammer constructor
    try:
        # Just use the ELF file - let Hammer figure out everything else
        sim = hammer.Hammer(elf_path)
    except Exception as e:
        error_msg = f"Hammer constructor with ELF only failed: {{str(e)}}"
...
'''
        ...
        try:
            ...
            return {"success": False, "message": f"Hammer initialization timeout. First message: {ready_msg}. Stderr: {stderr_output}"}
            
        except queue.Empty:
            # Check for subprocess errors
            stderr_output = self.process.stderr.read() if self.process.stderr else "No stderr"
            return {"success": False, "message": f"Hammer startup timeout. Stderr: {stderr_output}"}
